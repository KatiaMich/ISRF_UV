...
def exponent(HIPvsSH,pdtable,co,interpolated_dustMap,Alambda_overAv,dustTweek):
...
def compute(HIPvsSH,pdtable, wavelength,co,interpolated_dustMap,cwd,dustTweek):
    extinction_Function, g, Teff = importFiles(HIPvsSH,pdtable,cwd)   

    Alambda_overAv = extinctionRatio(wavelength,extinction_Function)
    expon = exponent(HIPvsSH, pdtable,co,interpolated_dustMap,Alambda_overAv,dustTweek)
...
def getFlux(HIPvsSH,filename, wavelength,co,interpolated_dustMap,cwd,dustTweek):
    ...
    F_O = compute(HIPvsSH,pdO, wavelength,co,interpolated_dustMap,cwd,dustTweek)
    F_B = compute(HIPvsSH,pdB, wavelength,co,interpolated_dustMap,cwd,dustTweek)
    F_A = compute(HIPvsSH,pdA, wavelength,co,interpolated_dustMap,cwd,dustTweek)
    return F_O, F_B, F_A
